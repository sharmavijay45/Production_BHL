...
# MongoDB client - initialized lazily to avoid circular import
_mongo_client = None
_mongo_db = None
...
def _get_mongo_collection():
    ...
    if _mongo_collection is None:
        try:
            ...
            _mongo_client = motor.motor_asyncio.AsyncIOMotorClient(MONGO_CONFIG['uri'])
            _mongo_db = _mongo_client[MONGO_CONFIG['database']]
            _mongo_collection = _mongo_db[MONGO_CONFIG['collection']]
...
async def log_to_mongo(task_id: str, agent: str, input_data: str, output_data: dict):
    ...
    try:
        ...
        if collection is None:
            logger = logging.getLogger(__name__)
            logger.warning(f"MongoDB not available, skipping log for task {task_id}")
            return
